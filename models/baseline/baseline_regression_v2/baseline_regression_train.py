...
from baseline.baseline_regression_v2.baseline_regresssion import *
from utilities.data_loader import *
from utilities.my_metrics import *
from utilities.my_loss import L1_loss
import pickle

def baseline_Regression(test_fold, total_folds, data_set_info_dict,
                       backbone_name, device, epoch_num, results_save_dir,
                        model_save_dir, criterion_type = 'L2'):
    criterion = None
    ...
    trainloader, testloader = get_train_test_data_loader(data_set_info_dict,
                                                         ...
                                                         sub_begin_age=False)
    min_age = data_set_info_dict['min_age']
    max_age = data_set_info_dict['max_age']
    model = BaselineRegressionModel(backbone_name)
    model = model.to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    model_save_dir = os.path.join(model_save_dir, criterion_type, 'testFold_' + str(test_fold))
    if not os.path.exists(model_save_dir):
        os.makedirs(model_save_dir)
    results_save_dir = os.path.join(results_save_dir, criterion_type)
...
def baseline_regression_process(model, trainloader, testloader, optimizer, epoch_num, device, log_file_name, model_save_dir,
    ...
    for epoch in range(epoch_num):
        for step, data in enumerate(trainloader, 0):
            images, labels = data
            labels = labels.float()
            labels = 2.0 * (labels - min_age) / (max_age - min_age) - 1.0
        ...
        with torch.no_grad():
            ...
            for _, test_data in enumerate(testloader, 0):
                ...
                predicted = 0.5 * (outputs + 1.0) * (max_age - min_age) + min_age
