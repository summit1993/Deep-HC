...
from DHC.Poker.Poker_V3.pokerModel_loss import *
...
def Poker_LDL_train(test_fold, total_folds, data_set_info_dict, hierarchy, label_HLDL_dict,
                       ...
                       use_wight=False, use_select=False, thread=0.0):
    ...
    poker_model_process(model, trainloader, testloader, optimizer, epoch_num, device,
                        hierarchy, label_HLDL_dict, begin_age, log_file_name, model_save_dir, use_wight, use_select, thread)

def poker_model_process(model, trainloader, testloader, optimizer, epoch_num, device,
                        hierarchy, label_HLDL_dict, begin_age, log_file_name, model_save_dir, use_weight, use_select, thread):
    results = {}
    results['MAE'] = []
    results['outputs'] = []
    ...
    for epoch in range(epoch_num):
        for step, data in enumerate(trainloader, 0):
            ...
            if use_weight:
                if use_select:
                    loss = pokerModel_calculate_loss_with_select(outputs, labels,
                                                                 hierarchy, label_HLDL_dict, device, thread)
                else:
                    loss = pokerModel_calculate_loss_with_weigt(outputs, labels,
                                                                hierarchy, label_HLDL_dict, device)
            else:
